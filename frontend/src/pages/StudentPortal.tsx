...
import { STUDENTS, STUDENT_SUBMISSIONS, GRADE_SCALE, SUBJECTS } from '../data/universityData';
...
const StudentPortal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Find student data
    const student = Object.values(STUDENTS).flat().find(s => s.id === userData.id);
    if (!student) {
      navigate('/login');
      return;
    }
  ...
  const calculateAverageScore = (): number => {
    if (!studentData.submissions || studentData.submissions.length === 0) return 0;
    
    const scores = studentData.submissions
      .filter((sub: any) => sub.score !== undefined)
                    ...
                    <Card variant="outlined">
                      <CardContent>
                        ...
                        {studentData.submissions.length > 0 ? (
                          <Box>
                            {studentData.submissions.map((submission: any, index: number) => (
                      ...
                      <TableBody>
                        {studentData.submissions.map((submission: any, index: number) => (
                          <TableRow key={index}>
                            ...
                            <TableCell>{getSubjectName(submission.subjectId)}</TableCell>
                            <TableCell>{submission.examType}</TableCell>
                            <TableCell align="right">{submission.score}</TableCell>
                            <TableCell align="right">{calculateGrade(submission.score)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                ...
                <TabPanel value={tabValue} index={1}>
                  ...
                  <Paper variant="outlined" sx={{ p: 3, bgcolor: 'background.default' }}>
                    ...
                    <Box component="ul" sx={{ pl: 2 }}>
                      ...
                      <Box component="li">
                        <Typography variant="body2">
                          Seek guidance from your professors during office hours.
                        </Typography>
                      </Box>
                    </Box>
                  </Paper>
